...
function IncidentCard({ r, compact, onEdit, onDelete, currentUid, isAdmin, userPos, incidents = [], userProfile }: { r: any, compact?: boolean, onEdit?: (r: any) => void, onDelete?: (id: string) => void, currentUid?: string, isAdmin?: boolean, userPos?: any, incidents?: any[], userProfile?: any }) {
  ...
  return (
    <div style={{ 
      ...
      marginBottom: 10, 
      border: isHighlyVerified ? "2px solid #E53935" : "1px solid #F0EFEB", 
      overflow: "hidden",
      boxShadow: isHighlyVerified ? "0 4px 20px rgba(229,57,53,0.15)" : "none"
    }}>
      {isHighlyVerified && (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap", marginBottom: 3 }}>
            ...
            {(r.verified || isHighlyVerified) && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 3, background: "#E8F5E9", borderRadius: 20, padding: "2px 6px" }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, color: "#2E7D32" }}>{isHighlyVerified ? "Highly Verified" : "Verified"}</span>
              </span>
            )}
            {r.solved && (
      ...
      <div style={{ padding: "8px 14px 12px" }}>
        ...
        {notes.length > 0 && (
          <div style={{ marginTop: 12, background: "#F9F9F7", borderRadius: 10, padding: 10, border: "1px solid #F0EFEB" }}>
            <div style={{ fontSize: 10, fontWeight: 800, color: "#999", marginBottom: 6, letterSpacing: "0.05em" }}>COMMUNITY NOTES ({notes.length})</div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                {canWatchBtn && (
                  <button onClick={handleWatch}
                    disabled={isWatching}
                    style={{
                      display: "flex", alignItems: "center", gap: 4, border: "none", background: isWatching ? "#E3F2FD" : "#F5F4F0",
                      borderRadius: 8, padding: "6px 10px",
                      cursor: isWatching ? "default" : "pointer", color: isWatching ? "#1E88E5" : "#666", fontSize: 11, fontWeight: 700
                    }}>
                ...
                {canConfirmBtn && (
                  <button onClick={handleConfirm}
                    ...
                    style={{
                      display: "flex", alignItems: "center", gap: 4, border: "none", background: isConfirmed ? "#E8F5E9" : "#F5F4F0",
                      borderRadius: 8, padding: "6px 10px",
                      cursor: isConfirmed ? "default" : "pointer", color: isConfirmed ? "#2E7D32" : "#0A1628", fontSize: 11, fontWeight: 800
                    }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                    {confirms.length}
                  </button>
                )}
                <button onClick={() => setShowNoteInput(!showNoteInput)} style={{ display: "flex", alignItems: "center", gap: 4, background: "#F5F4F0", border: "none", borderRadius: 8, padding: "6px 10px", color: "#1E88E5", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                  Note
                </button>
                {canResolve && (
                  <button onClick={handleResolve} style={{ display: "flex", alignItems: "center", gap: 4, background: r.solved ? "#E8F5E9" : "#F5F4F0", border: "none", borderRadius: 8, padding: "6px 10px", color: r.solved ? "#2E7D32" : "#43A047", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
              ...
              {isOwner && (
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => onEdit?.(r)} style={{ background: "#F5F4F0", border: "none", borderRadius: 8, padding: "6px 10px", color: "#1E88E5", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Edit</button>
                  <button onClick={() => onDelete?.(r.id)} style={{ background: "#FFF1F0", border: "none", borderRadius: 8, padding: "6px 10px", color: "#E53935", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Delete</button>
                </div>
              )}
            </div>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {allSignals.map((s: any) => (
                <div key={s.id} onClick={() => { setTab("profile"); setTimeout(() => { document.getElementById('sos-section')?.scrollIntoView({ behavior: 'smooth' }); }, 100); }} style={{ background: "#B71C1C", borderRadius: 16, padding: 16, border: "2px solid #E53935", cursor: "pointer", boxShadow: "0 8px 24px rgba(183,28,28,0.2)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
...
function MapTab({ selDistName, setDistName, districts, incidents, geofences, loading, currentUid, isAdmin, onEdit, onDelete, sosSignals = [] }: any) {
...
function FeedTab({ incidents, currentUid, isAdmin, onEdit, userPos, userProfile }: any) {
...
function ProfileTab({ user, userRole, userProfile, incidents, geofences, districts, contactSosSignals, userPos, onEdit, onOpenAdmin }: any) {
  ...
  const [newName, setNewName] = useState(userProfile?.displayName || user?.displayName || "");
  const [newPhoto, setNewPhoto] = useState(userProfile?.photoURL || user?.photoURL || "");
    ...
    <div style={{ fontFamily: "'DM Sans','Helvetica Neue',sans-serif", background: "#F5F4F0", minHeight: "100vh", maxWidth: 430, margin: "0 auto", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ overflowY: "auto", paddingBottom: 82, height: "calc(100vh - 140px)" }}>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {tab === "home" && <HomeTab setTab={setTab} setDist={(n: string) => { setSelDistName(n); setTab("map"); }} incidents={incidents} news={news} liveAlerts={liveAlerts} aiInsights={aiInsights} geofences={geofences} currentUid={user?.uid} isAdmin={userRole === "admin"} onEdit={(r: any) => { setEditIncident(r); setTab("report"); }} onDelete={async (id: string) => { if (window.confirm("Delete this report?")) await deleteDoc(doc(db, "incidents", id)); }} user={user} userPos={userPos} userProfile={userProfile} activeSosId={activeSosId} onActiveSosId={setActiveSosId} polls={polls} contactSosSignals={contactSosSignals} />}
            {tab === "feed" && <FeedTab incidents={incidents} currentUid={user?.uid} isAdmin={userRole === "admin"} onEdit={(r: any) => { setEditIncident(r); setTab("report"); }} userPos={userPos} userProfile={userProfile} />}
            {tab === "map" && <MapTab selDistName={selDistName} setDistName={setSelDistName} districts={districts} incidents={incidents} geofences={geofences} loading={districtsLoading} currentUid={user?.uid} isAdmin={userRole === "admin"} onEdit={(r: any) => { setEditIncident(r); setTab("report"); }} onDelete={async (id: string) => { if (window.confirm("Delete this report?")) await deleteDoc(doc(db, "incidents", id)); }} sosSignals={contactSosSignals} />}
