...
const GROUPS = [
  ...
  { id: "hazard", label: "Hazard", icon: "△", color: "#E65100", bg: "#FFF8E1", desc: "Fire, flood, road damage" },
  { id: "welfare", label: "Welfare", icon: "♡", color: "#1E88E5", bg: "#E3F2FD", desc: "Missing person, medical" },
];
...
function IncidentCard({ r, compact, onEdit, onDelete, currentUid, isAdmin }: { r: any, compact?: boolean, onEdit?: (r: any) => void, onDelete?: (id: string) => void, currentUid?: string, isAdmin?: boolean }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: compact ? 12 : 16, marginBottom: 10, border: "1px solid #F0EFEB", overflow: "hidden" }}>
      {r.img && (
        <div style={{ width: "100%", height: 180, overflow: "hidden", background: "#F0EFEB", position: "relative" }}>
      ...
      <div style={{ padding: "12px 14px 8px", display: "flex", alignItems: "flex-start", gap: 10 }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: col, fontWeight: 700, flexShrink: 0 }}>
          {gIcon(r.type)}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap", marginBottom: 3 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#0A1628" }}>{r.anon ? "Anonymous" : r.user}</span>
            {r.trusted && !r.anon && <TrustedBadge />}
            {r.verified && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 3, background: "#E8F5E9", borderRadius: 20, padding: "2px 6px" }}>
                <span style={{ color: "#2E7D32", fontSize: 10 }}>✓</span>
                <span style={{ fontSize: 10, fontWeight: 700, color: "#2E7D32" }}>Verified</span>
              </span>
            )}
            {r.solved && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 3, background: "#E3F2FD", borderRadius: 20, padding: "2px 6px" }}>
                <span style={{ color: "#1976D2", fontSize: 10 }}>✓</span>
                <span style={{ fontSize: 10, fontWeight: 700, color: "#1976D2" }}>Solved</span>
              </span>
            )}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span style={{ color: "#CCC", display: "flex" }}><IcPin /></span>
            <span style={{ fontSize: 11, color: "#999" }}>{r.district}</span>
        ...
        <span style={{ background: bg, color: col, fontSize: 9, fontWeight: 700, padding: "3px 7px", borderRadius: 6, letterSpacing: "0.04em", flexShrink: 0 }}>
      ...
      <div style={{ padding: "8px 14px 12px" }}>
        <p style={{
          ...
          display: "-webkit-box", WebkitLineClamp: compact ? 2 : 5, WebkitBoxOrient: "vertical", overflow: "hidden"
        }}>
          {r.desc}
        </p>

        {r.notes && r.notes.length > 0 && (
          <div style={{ marginTop: 12, background: "#F9F9F7", borderRadius: 10, padding: 10, border: "1px solid #F0EFEB" }}>
            <div style={{ fontSize: 10, fontWeight: 800, color: "#999", marginBottom: 6, letterSpacing: "0.05em" }}>COMMUNITY NOTES</div>
            {r.notes.map((n: any, i: number) => (
              <div key={i} style={{ fontSize: 11, color: "#555", marginBottom: 4, lineHeight: 1.4 }}>
                <span style={{ fontWeight: 700, color: "#0A1628" }}>{n.user}:</span> {n.text}
              </div>
            ))}
          </div>
        )}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <button onClick={handleLike}
                  style={{
                    ...
                    cursor: "pointer", color: liked ? "#E53935" : "#BBB", fontSize: 12, fontWeight: 600, padding: 0
                  }}>
                  <IcHeart />{r.likes || 0}
                </button>
                <button onClick={() => setShowNoteInput(!showNoteInput)} style={{ background: "none", border: "none", color: "#1E88E5", fontSize: 11, fontWeight: 700, cursor: "pointer", padding: 0 }}>
                  + Add Note
                </button>
                {canResolve && (
                  <button onClick={handleResolve} style={{ background: "none", border: "none", color: r.solved ? "#999" : "#43A047", fontSize: 11, fontWeight: 700, cursor: "pointer", padding: 0 }}>
              ...
              {isOwner && (
                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={() => onEdit?.(r)} style={{ background: "none", border: "none", color: "#1E88E5", fontSize: 11, fontWeight: 700, cursor: "pointer", padding: 0 }}>Edit</button>
                  <button onClick={() => onDelete?.(r.id)} style={{ background: "none", border: "none", color: "#E53935", fontSize: 11, fontWeight: 700, cursor: "pointer", padding: 0 }}>Delete</button>
                </div>
              )}
            </div>
            ...
            {showNoteInput && (
              <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                <input 
                  ...
                  placeholder="Add context or update..." 
                  style={{ flex: 1, borderRadius: 8, border: "1px solid #EEECEA", padding: "6px 10px", fontSize: 12, outline: "none" }}
                />
                <button onClick={handleAddNote} style={{ background: "#0A1628", color: "#fff", border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Add</button>
              </div>
            )}
          </div>
        )}
...
// --- TABS ---
function HomeTab({ setTab, setDist, incidents, news, liveAlerts, aiInsights, geofences = [], currentUid, isAdmin, onEdit, onDelete, user, userPos, userProfile, activeSosId, onActiveSosId, polls = [] }: any) {
  const top3 = useMemo(() => {
    const counts: any = {};
      ...
      <div style={{ padding: "20px 16px" }}>
        ...
        {polls.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {polls.filter((p: any) => p.active).slice(0, 2).map((p: any) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {incidents.length > 0 ? incidents.map((r: any) => (
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: 40, textAlign: "center", border: "1px solid #F0EFEB" }}>
...
function MogadishuMap({ districts, incidents, geofences = [], sosSignals = [], onSelect, onPick, mini, center, zoom }: any) {
  ...
  return (
    <div style={{ 
      ...
      <MapContainer center={mapCenter} zoom={mapZoom} style={{ height: "100%", width: "100%" }} scrollWheelZoom={!mini}>
        ...
        {!mini && districts.map((d: any) => {
          ...
          return (
            <Circle
              ...
              radius={500}
              pathOptions={{ color: rc.color, fillColor: rc.color, fillOpacity: 0.4 }}
              eventHandlers={{
            ...
            >
              <Popup>
                ...
                <div style={{ fontSize: 10 }}>Risk: {d.risk.toUpperCase()}</div>
              </Popup>
            </Circle>
          );
        })}
        ...
        {/* Incidents */}
        {!mini && incidents.filter((inc: any) => inc.location && typeof inc.location.lat === 'number' && typeof inc.location.lng === 'number').map((inc: any) => {
          const col = gColor(inc.type);
          return (
            <Circle
              key={inc.id}
              center={[inc.location.lat, inc.location.lng]}
              radius={50}
              pathOptions={{ color: col, fillColor: col, fillOpacity: 0.8 }}
            >
              <Popup>
...
function MapTab({ selDistName, setDistName, districts, incidents, geofences, loading, currentUid, isAdmin, onEdit, onDelete }: any) {
  const districtData = useMemo(() => {
    return districts.map((d: any) => {
      const distIncidents = incidents.filter((i: any) => i.district === d.name);
      return { 
        ...d, 
        incidents: distIncidents.length, 
        active: distIncidents.filter((i: any) => !i.solved).length,
        score: Math.max(0, 100 - (distIncidents.length * 5) - (distIncidents.filter((i: any) => i.type === 'armed').length * 10))
      };
    });
  }, [districts, incidents]);
    ...
    return (
      <div style={{ background: "#F5F4F0", minHeight: "100vh" }}>
        ...
        <div style={{ padding: "16px" }}>
          ...
          {distReps.length ? distReps.map((r: any) => <IncidentCard key={r.id} r={r} currentUid={currentUid} isAdmin={isAdmin} onEdit={onEdit} onDelete={onDelete} />) : (
  ...
  return (
    <div>
      <div style={{ background: "#0A1628", padding: "16px 16px 14px" }}>
        ...
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", marginBottom: 16 }}>Real-time risk heatmap of Mogadishu</div>
        <MogadishuMap districts={districts} incidents={incidents} geofences={geofences} onSelect={setDistName} />
      </div>
...
function FeedTab({ incidents, currentUid, isAdmin, onEdit }: any) {
...
function PollCard({ poll, isAdmin, onEdit }: { poll: any, isAdmin?: boolean, onEdit?: () => void }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 16, padding: 16, border: "1px solid #F0EFEB", boxShadow: "0 2px 12px rgba(0,0,0,0.03)" }}>
      <div style={{ fontSize: 15, fontWeight: 800, color: "#0A1628", marginBottom: 4, lineHeight: 1.4 }}>{poll.q}</div>
      <div style={{ fontSize: 11, color: "#999", marginBottom: 16 }}>{poll.en}</div>
        ...
        {poll.opts.map((o: any, i: number) => {
          ...
          return (
            <div key={i} onClick={() => handleVote(i)}
              style={{ marginBottom: 4, borderRadius: 12, overflow: "hidden", border: isV ? "1.5px solid #0A1628" : "1.5px solid #F0EFEB", cursor: voted !== null ? "default" : "pointer", position: "relative" }}>
              <div style={{ position: "absolute", top: 0, left: 0, height: "100%", width: voted !== null ? pct + "%" : "0%", background: isV ? "#0A162810" : "#F5F4F0", transition: "width 0.6s ease" }} />
              <div style={{ position: "relative", display: "flex", justifyContent: "space-between", padding: "12px 14px" }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#0A1628" }}>{o.l}</span>
                {voted !== null && <span style={{ fontSize: 12, fontWeight: 800, color: isV ? "#0A1628" : "#999" }}>{pct}%</span>}
              </div>
            </div>
          );
        })}
      </div>
      <div style={{ fontSize: 11, color: "#BBB", marginTop: 12, display: "flex", justifyContent: "space-between" }}>
...
function NewsTab({ news, polls, sosSignals, isAdmin, onEnter }: any) {
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ background: "#fff", borderRadius: 16, padding: 16, border: "1px solid #F0EFEB", boxShadow: "0 2px 12px rgba(0,0,0,0.03)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                {[
                  { tag: "Hodan", count: 124, trend: "up" },
                  { tag: "SafetyFirst", count: 89, trend: "up" },
                  { tag: "Mogadishu", count: 245, trend: "stable" },
                  { tag: "Peace", count: 56, trend: "up" },
                ].map((t, i) => (
                  <div key={t.tag} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 15, fontWeight: 700, color: "#0A1628" }}>#{t.tag}</div>
                        <div style={{ fontSize: 11, color: "#999", marginTop: 2 }}>{t.count} reports this week</div>
                      </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 4, color: t.trend === "up" ? "#E53935" : "#999", fontSize: 12, fontWeight: 800 }}>
                      {t.trend === "up" ? "▲" : "—"}
                      {t.trend === "up" && <span style={{ fontSize: 9 }}>HOT</span>}
                    </div>
                  </div>
                ))}
              </div>
...
function NearbyAlertsModal({ userPos, incidents, liveAlerts, isAdmin, onClose }: any) {
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div>
              ...
              {nearbyIncidents.length > 0 ? nearbyIncidents.map((r: any) => (
                <IncidentCard key={r.id} r={r} compact />
              )) : (
  ...
  const handleSave = async () => {
    ...
    try {
      if (type === "news") {
        ...
        const data = { q, en: "", opts: options, active: true };
        if (editItem?.id) {
  ...
  return (
    <div style={{ background: "#F5F4F0", padding: 16, borderBottom: "1px solid #EEECEA" }}>
      <div style={{ display: "flex", gap: 6, marginBottom: 14, overflowX: "auto", paddingBottom: 4 }}>
        ...
        <button onClick={() => setType("sos")} style={{ flexShrink: 0, padding: "8px 16px", borderRadius: 8, border: type === "sos" ? "none" : "1px solid #CCC", background: type === "sos" ? "#B71C1C" : "#fff", color: type === "sos" ? "#fff" : "#666", fontSize: 11, fontWeight: 700 }}>SOS Signals ({sosSignals?.filter((s: any) => s.status === "active").length || 0})</button>
      </div>
      ...
      {type === "sos" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {sosSignals?.length === 0 ? (
            <div style={{ padding: 20, textAlign: "center", color: "#999", fontSize: 13 }}>No SOS signals reported.</div>
          ) : (
            sosSignals.map((s: any) => (
              <div key={s.id} style={{ background: s.status === "active" ? "#FFEBEE" : "#fff", borderRadius: 12, padding: 12, border: "1px solid #EEECEA" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: s.status === "active" ? "#B71C1C" : "#666" }}>{s.user || "Unknown User"}</div>
                  <div style={{ fontSize: 10, color: "#999" }}>{s.timestamp?.toDate().toLocaleTimeString()}</div>
                </div>
                <div style={{ fontSize: 12, color: "#333", marginBottom: 8 }}>Location: {s.lat.toFixed(5)}, {s.lng.toFixed(5)}</div>
                {s.status === "active" && (
                  <button onClick={() => resolveSOS(s.id)} style={{ background: "#43A047", color: "#fff", border: "none", borderRadius: 6, padding: "4px 12px", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>Mark Resolved</button>
                )}
                {s.status === "resolved" && (
                  <span style={{ fontSize: 10, color: "#43A047", fontWeight: 700 }}>RESOLVED</span>
                )}
              </div>
            ))
          )}
...
function SOSContactsManager({ contacts, onUpdate }: { contacts: any[], onUpdate: (c: any[]) => void }) {
  ...
  const handleAdd = () => {
    ...
    onUpdate([...contacts, { name, email: email.toLowerCase() }]);
    setName("");
    setEmail("");
  };
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 16, padding: 16, border: "1px solid #F0EFEB", marginBottom: 20 }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: "#0A1628", marginBottom: 12 }}>Emergency SOS Contacts (Max 3)</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
        {contacts.map((c, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#F5F4F0", padding: "8px 12px", borderRadius: 10 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700 }}>{c.name}</div>
              <div style={{ fontSize: 11, color: "#999" }}>{c.email}</div>
            </div>
            <button onClick={() => handleRemove(i)} style={{ background: "none", border: "none", color: "#E53935", fontSize: 18, cursor: "pointer" }}>×</button>
          </div>
        ))}
      </div>
      {contacts.length < 3 && (
        <div style={{ display: "flex", gap: 8 }}>
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Name" style={{ flex: 1, padding: 8, borderRadius: 8, border: "1px solid #EEECEA", fontSize: 12 }} />
          <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Gmail/Email" style={{ flex: 1, padding: 8, borderRadius: 8, border: "1px solid #EEECEA", fontSize: 12 }} />
          <button onClick={handleAdd} style={{ background: "#0A1628", color: "#fff", border: "none", borderRadius: 8, padding: "0 12px", fontSize: 12, fontWeight: 700 }}>Add</button>
        </div>
      )}
    </div>
  );
...
function SafetySettings({ user, userProfile, geofences, userPos, districts, incidents, onUpdateContacts, onAddGeofence, onDeleteGeofence, contactSosSignals }: any) {
  return (
    <div style={{ background: "#fff", borderRadius: 24, padding: 20, border: "1px solid #F0EFEB", marginBottom: 24 }}>
            ...
            {contactSosSignals.map((s: any) => (
              <div key={s.id} style={{ background: "#FFEBEE", borderRadius: 16, padding: 16, border: "1px solid #FFCDD2" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 10, color: "#E57373" }}>{s.timestamp?.toDate().toLocaleTimeString()}</div>
                </div>
                <div style={{ fontSize: 12, color: "#D32F2F", marginBottom: 12 }}>Emergency signal active! View location on map.</div>
                <div style={{ height: 120, background: "#0A1628", borderRadius: 12, overflow: "hidden", position: "relative" }}>
                   <MogadishuMap 
                     districts={[]} 
                     incidents={[]} 
                     geofences={[]} 
...
function ProfileTab({ user, userRole, userProfile, incidents, geofences, districts, contactSosSignals, userPos, onEdit, onOpenAdmin }: any) {
  return (
    <div style={{ padding: 20 }}>
      <div style={{ textAlign: "center", marginBottom: 32 }}>
        <div style={{ width: 80, height: 80, borderRadius: "50%", background: "#0A1628", margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 32, fontWeight: 900, border: "4px solid #fff", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}>
          {user.displayName?.[0] || "U"}
        </div>
        <div style={{ fontSize: 20, fontWeight: 800, color: "#0A1628" }}>{user.displayName}</div>
        <div style={{ fontSize: 14, color: "#999" }}>{user.email}</div>
        <div style={{ marginTop: 8 }}>
          <span style={{ background: userRole === "admin" ? "#B71C1C" : "#0A1628", color: "#fff", padding: "4px 12px", borderRadius: 20, fontSize: 10, fontWeight: 900, letterSpacing: "0.05em" }}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
        <div style={{ flex: 1, background: "#fff", borderRadius: 16, padding: 16, border: "1px solid #F0EFEB", textAlign: "center" }}>
          <div style={{ fontSize: 20, fontWeight: 900, color: "#0A1628" }}>{incidents.filter((i: any) => i.uid === user.uid).length}</div>
          <div style={{ fontSize: 10, color: "#999", fontWeight: 700 }}>REPORTS</div>
        </div>
        <div style={{ flex: 1, background: "#fff", borderRadius: 16, padding: 16, border: "1px solid #F0EFEB", textAlign: "center" }}>
          <div style={{ fontSize: 20, fontWeight: 900, color: "#E8C547" }}>{geofences.length}</div>
          <div style={{ fontSize: 10, color: "#999", fontWeight: 700 }}>ZONES</div>
        </div>
      </div>
      ...
      <div style={{ textAlign: "left", marginBottom: 24 }}>
        <div style={{ fontSize: 14, fontWeight: 900, color: "#0A1628", marginBottom: 12, letterSpacing: "-0.2px" }}>My Recent Reports</div>
        {incidents.filter((i: any) => i.uid === user.uid).length === 0 ? (
          <div style={{ padding: 24, background: "#fff", borderRadius: 16, border: "1px dashed #DDD", textAlign: "center", color: "#999", fontSize: 13 }}>
      ...
      {userRole === "admin" && (
        <div style={{ marginTop: 24, padding: 20, background: "#fff", borderRadius: 16, border: "1px solid #F0EFEB", textAlign: "left" }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#0A1628", marginBottom: 12 }}>Admin Actions</div>
          <button 
            onClick={onOpenAdmin}
            style={{ width: "100%", background: "#0A1628", color: "#fff", border: "none", borderRadius: 12, padding: "12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}
          >
            Open Admin Panel
          </button>
        </div>
      ...
      <button 
        ...
        style={{ width: "100%", marginTop: 32, background: "none", border: "1.5px solid #EEECEA", borderRadius: 14, padding: 14, fontSize: 14, fontWeight: 700, color: "#E53935", cursor: "pointer" }}
      >
        Sign Out
      </button>
    </div>
...
function AppContent() {
  ...
  // Sync SOS Signals for admins
  useEffect(() => {
    if (userRole === "admin") {
      const q = query(collection(db, "sos_signals"), orderBy("timestamp", "desc"), limit(20));
      const unsub = onSnapshot(q, (snap) => {
        setSosSignals(snap.docs.map(d => ({ id: d.id, ...d.data() })));
      }, (err) => handleFirestoreError(err, OperationType.LIST, "sos_signals"));
      return unsub;
    }
  }, [userRole]);
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (user.email) {
        ...
        const qContactSos = query(collection(db, "sos_signals"), where("status", "==", "active"), where("contactEmails", "array-contains", email));
        unsubContactSos = onSnapshot(qContactSos, (snap) => {
          setContactSosSignals(snap.docs.map(d => ({ id: d.id, ...d.data() })));
        });
      }
    }
    ...
    return () => { 
      unsubInc(); unsubDist(); unsubNews(); unsubPolls(); unsubMessages(); unsubAlerts(); unsubGeofences(); unsubNotifications(); unsubContactSos();
  ...
  useEffect(() => {
    ...
    const seedData = async () => {
      ...
      if (news.length === 0) {
        const defaultNews = [
          { title: "Security Alert: Bakara Market Area", cat: "ALERT", district: "Howlwadaag", urgent: true, content: "Increased security presence reported near Bakara Market. Citizens are advised to avoid unnecessary travel to the area.", reads: 1240, timestamp: serverTimestamp() },
          { title: "Community Meeting in Hodan", cat: "COMMUNITY", district: "Hodan", urgent: false, content: "A community safety meeting will be held this Saturday at the district office. All residents are welcome.", reads: 450, timestamp: serverTimestamp() },
          { title: "New Road Safety Measures", cat: "UPDATE", district: "Waberi", urgent: false, content: "Local authorities have implemented new traffic control measures on the main airport road to improve safety.", reads: 890, timestamp: serverTimestamp() },
          { title: "Garasbaley Infrastructure Update", cat: "UPDATE", district: "Garasbaley", urgent: false, content: "New street lighting project launched in Garasbaley to improve nighttime safety and visibility.", reads: 320, timestamp: serverTimestamp() },
          { title: "Emergency Contact List Updated", cat: "NOTICE", district: "All", urgent: true, content: "The list of emergency contact numbers for all 18 districts has been updated. Please check the safety section.", reads: 2100, timestamp: serverTimestamp() },
        ];
        try {
          const promises = defaultNews.map(n => addDoc(collection(db, "news"), n));
      ...
      if (polls.length === 0) {
        const defaultPolls = [
          { q: "How safe do you feel in your district today?", en: "Community Safety Sentiment", opts: [{ l: "Very Safe", v: 12 }, { l: "Somewhat Safe", v: 45 }, { l: "Unsafe", v: 8 }], total: 65, active: true, timestamp: serverTimestamp() },
          { q: "Should we increase night patrols in commercial areas?", en: "Security Resource Allocation", opts: [{ l: "Yes, definitely", v: 89 }, { l: "No, focus on day", v: 12 }, { l: "Unsure", v: 5 }], total: 106, active: true, timestamp: serverTimestamp() },
          { q: "What is the most urgent safety concern in your area?", en: "Priority Identification", opts: [{ l: "Street Crime", v: 34 }, { l: "Traffic Safety", v: 21 }, { l: "Infrastructure", v: 15 }], total: 70, active: true, timestamp: serverTimestamp() },
        ];
        try {
          const promises = defaultPolls.map(p => addDoc(collection(db, "polls"), p));
          await Promise.all(promises);
        } catch (err) {
          console.error("Polls seeding failed", err);
        }
      }
    };
  ...
  return (
    <div style={{ fontFamily: "'DM Sans','Helvetica Neue',sans-serif", background: "#F5F4F0", minHeight: "100vh", maxWidth: 430, margin: "0 auto", position: "relative", overflow: "hidden" }}>
      ...
      <AnimatePresence>
        {showNearby && <NearbyAlertsModal userPos={userPos} incidents={incidents} liveAlerts={liveAlerts} isAdmin={userRole === "admin"} onClose={() => setShowNearby(false)} />}
      </AnimatePresence>

      <div style={{ overflowY: "auto", paddingBottom: 82, height: "calc(100vh - 140px)" }}>
        <AnimatePresence>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {tab === "home" && <HomeTab setTab={setTab} setDist={(n: string) => { setSelDistName(n); setTab("map"); }} incidents={incidents} news={news} liveAlerts={liveAlerts} aiInsights={aiInsights} geofences={geofences} currentUid={user?.uid} isAdmin={userRole === "admin"} onEdit={(r: any) => { setEditIncident(r); setTab("report"); }} onDelete={async (id: string) => { if (window.confirm("Delete this report?")) await deleteDoc(doc(db, "incidents", id)); }} user={user} userPos={userPos} userProfile={userProfile} activeSosId={activeSosId} onActiveSosId={setActiveSosId} polls={polls} />}
            {tab === "map" && <MapTab selDistName={selDistName} setDistName={setSelDistName} districts={districts} incidents={incidents} geofences={geofences} loading={districtsLoading} currentUid={user?.uid} isAdmin={userRole === "admin"} onEdit={(r: any) => { setEditIncident(r); setTab("report"); }} onDelete={async (id: string) => { if (window.confirm("Delete this report?")) await deleteDoc(doc(db, "incidents", id)); }} />}
            ...
            {tab === "news" && <NewsTab news={news} polls={polls} sosSignals={sosSignals} isAdmin={userRole === "admin"} onEnter={() => setLastViewedNews(Date.now())} />}
            {tab === "profile" && (user ? (
              <ProfileTab 
                user={user} 
                userRole={userRole} 
                ...
                contactSosSignals={contactSosSignals} 
                userPos={userPos}
                onEdit={(r: any) => { setEditIncident(r); setTab("report"); }} 
                onOpenAdmin={() => {
                  setTab("news");
                  setTimeout(() => {
