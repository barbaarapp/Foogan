...
import { useState, useEffect, useMemo, memo, useRef, Suspense, lazy } from "react";
  ...
  return (
    <div ref={viewRef} style={{ 
      ...
      <div style={{ padding: "8px 4px 12px" }}>
        ...
        {isAcceptedByMe && r.location && (
          <div style={{ marginTop: 12, borderRadius: 16, overflow: "hidden", border: "1px solid #F0EFEB" }}>
            <div style={{ background: "#0A1628", padding: "8px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: "#fff" }}>ROUTE TO VICTIM</span>
              <button 
                onClick={() => window.open(`https://www.google.com/maps/dir/?api=1&destination=${r.location.lat},${r.location.lng}`, '_blank')}
                style={{ background: "#E8C547", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, fontWeight: 800, cursor: "pointer" }}
              >
                Open Maps
              </button>
            </div>
          ...
          <div style={{ marginTop: 12, background: "#F8F9FA", borderRadius: 12, padding: "12px", border: "1px solid #F0EFEB" }}>
            ...
            {notes.length > 2 && (
              <button 
                ...
                style={{ background: "none", border: "none", color: "#1E88E5", fontSize: 11, fontWeight: 800, cursor: "pointer", padding: 0, marginTop: 4 }}
              >
                Read all {notes.length} notes...
              </button>
            )}
          </div>
        )}
              ...
              >
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  {notes.map((n: any, i: number) => (
                    <div key={i} style={{ background: "#F8F9FA", padding: 14, borderRadius: 16, border: "1px solid #F0EFEB" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                        <div style={{ fontSize: 13, fontWeight: 800, color: "#0A1628" }}>{n.user}</div>
                        {n.uid === currentUid && (
        ...
        {!compact && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0", borderTop: "1px solid #F0EFEB", borderBottom: "1px solid #F0EFEB" }}>
            ...
            <AnimatePresence>
              {showNoteInput && (
                ...
                >
                  <div style={{ background: "#F8F9FA", padding: 12, borderRadius: 16, border: "1px solid #F0EFEB", marginTop: 8 }}>
                    <textarea 
...
// --- TABS ---
function HomeTab({ setTab, setDist, incidents, news, liveAlerts, aiInsights, geofences = [], currentUid, isAdmin, onEdit, onDelete, user, userPos, userProfile, activeSosId, onActiveSosId, polls = [], contactSosSignals = [], sponsors = [], onWatchMyBack }: any) {
  ...
  const feedItems = useMemo(() => {
    ...
    if (sponsors.length > 0) {
      ...
      let sponsorIdx = 0;
      // Ensure at least one sponsor if any exist and there are some incidents
...
function MogadishuMap({ districts, incidents, geofences = [], sosSignals = [], onSelect, onPick, mini, center, zoom, layer = "live", userPos }: any) {
  const mapCenter: [number, number] = (center && typeof center.lat === 'number' && typeof center.lng === 'number') 
    ...
    : (userPos ? [userPos.lat, userPos.lng] : [2.0469, 45.3182]);
  ...
  return (
    <div style={{ 
      ...
      <MapContainer center={mapCenter} zoom={mapZoom} style={{ height: "100%", width: "100%" }} scrollWheelZoom={!mini}>
        ...
        {/* Routing Polyline for mini map response */}
        {mini && userPos && incidents[0]?.location && (
          <Polyline 
            positions={[
              [userPos.lat, userPos.lng],
              [incidents[0].location.lat, incidents[0].location.lng]
            ]}
            pathOptions={{ color: '#1E88E5', weight: 4, dashArray: '8, 8', opacity: 0.8 }}
          />
        )}
...
function NewsCard({ n, catColors, isAdmin, setEditItem, setShowAdmin }: { n: any, catColors: any, isAdmin: boolean, setEditItem: any, setShowAdmin: any }) {
  ...
  return (
    <div ref={viewRef} style={{ background: "#fff", borderRadius: 16, overflow: "hidden", border: "1px solid #F0EFEB", boxShadow: "0 2px 12px rgba(0,0,0,0.03)" }}>
...
function NewsTab({ news, polls, sosSignals, isAdmin, onEnter, incidents = [], currentUid, sponsors = [] }: any) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F5F4F0" }}>
      ...
      {showAdmin && isAdmin && <AdminPanel editItem={editItem} sosSignals={sosSignals} incidents={incidents} sponsors={sponsors} onClose={() => { setShowAdmin(false); setEditItem(null); }} />}
...
function NearbyAlertsModal({ userPos, incidents, liveAlerts, isAdmin, onClose, currentUid, onEdit, onDelete, userProfile, notifications = [] }: any) {
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {notifications.length > 0 ? notifications.map((n: any) => (
                <div 
                  ...
                  style={{ background: "#fff", padding: 16, borderRadius: 16, border: n.read ? "1px solid #EEECEA" : "1.5px solid #E8C547", cursor: "pointer", position: "relative" }}
                >
                  {!n.read && <div style={{ position: "absolute", top: 12, right: 12, width: 8, height: 8, borderRadius: "50%", background: "#E53935" }} />}
                  <div style={{ fontSize: 11, fontWeight: 800, color: "#E8C547", marginBottom: 4 }}>{n.title}</div>
                  <div style={{ fontSize: 13, color: "#0A1628", fontWeight: 700, marginBottom: 4 }}>{n.body}</div>
                  <div style={{ fontSize: 11, color: "#999" }}>{n.timestamp?.toDate ? n.timestamp.toDate().toLocaleString() : ""}</div>
                </div>
              )) : (
...
function AdminPanel({ editItem, sosSignals, incidents, onClose, sponsors = [] }: any) {
  ...
  return (
    <div style={{ background: "#F5F4F0", padding: 16, borderBottom: "1px solid #EEECEA" }}>
      ...
      {type === "report" && (
        <ReportPanel incidents={incidents} sosSignals={sosSignals} />
      )}
...
function ProfileTab({ user, userRole, userProfile, incidents, geofences, districts, contactSosSignals, userPos, onEdit, onDelete, onOpenAdmin, onWatchMyBack }: any) {
  ...
  return (
    <div style={{ padding: "0 16px 24px" }}>
      ...
      <button 
        onClick={() => {
          // Find the function in AppContent and call it
          // Since we are in a child component, we need to pass it down
          onWatchMyBack?.();
        }}
        style={{ 
          width: "100%", background: "#0A1628", color: "#fff", border: "none", 
          borderRadius: 20, padding: "16px", display: "flex", alignItems: "center", 
          justifyContent: "center", gap: 12, cursor: "pointer", marginBottom: 24,
          boxShadow: "0 8px 20px rgba(10,22,40,0.2)"
        }}
      >
        <ShieldPlus size={24} color="#1E88E5" />
        <span style={{ fontSize: 14, fontWeight: 900, letterSpacing: "0.05em" }}>WATCH MY BACK REQUEST</span>
      </button>
            ...
            >
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 800, color: "#999", display: "block", marginBottom: 6, letterSpacing: "0.05em" }}>HOME DISTRICT</label>
                    <select 
                    ...
                    <div style={{ fontSize: 10, color: "#999", marginTop: 4 }}>* Residents can verify incidents in their own district.</div>
                  </div>
                  <button 
  ...
  const handleRequest = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "buddy_requests"), {
        ...
        buddyEmail: (selectedBuddy.email || selectedBuddy.value || "").toLowerCase(),
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 3000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {buddies.length === 0 ? (
              <div style={{ fontSize: 12, color: "#E53935", padding: 12, background: "#FFEBEE", borderRadius: 12 }}>
                No emergency contacts found. Add them in your profile first.
              </div>
            ) : (
              buddies.map((b: any, i: number) => (
                <button 
                  ...
                  style={{ 
                    ...
                    border: selectedBuddy === b ? "2px solid #1E88E5" : "1.5px solid #F0EFEB",
                    background: selectedBuddy === b ? "#E3F2FD" : "#fff",
                ...
                >
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 800, color: "#0A1628" }}>{b.name}</div>
...
function BuddyWatchOverlay({ request, user, onPulse, onStop }: any) {
  ...
  return (
    <motion.div 
      ...
      style={{ 
        ...
        background: "#0A1628", borderRadius: 24, padding: 16, color: "#fff",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ position: "relative" }}>
            <ShieldPlus size={20} color="#1E88E5" />
            <motion.div 
              animate={{ scale: [1, 1.5, 1], opacity: [0.5, 0, 0.5] }}
              transition={{ repeat: Infinity, duration: 2 }}
              style={{ position: "absolute", inset: -4, borderRadius: "50%", border: "2px solid #1E88E5" }}
            />
          </div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 900 }}>Watch My Back</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>
              {isUser ? `Buddy: ${request.buddyName}` : `User: ${request.userName}`} • {request.district}
            </div>
          </div>
        </div>
        <div style={{ fontSize: 11, fontWeight: 800, background: "#1E88E5", padding: "4px 8px", borderRadius: 6 }}>ACTIVE</div>
      </div>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        {isBuddy && (
          <button 
            onClick={onPulse}
            style={{ 
              flex: 2, background: "#E8C547", color: "#0A1628", border: "none", 
              borderRadius: 14, padding: 12, fontSize: 13, fontWeight: 900, 
              cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8
            }}
          >
            <HeartPulse size={18} />
...
function BuddyRequestAlert({ request, onAccept, onReject }: any) {
  return (
    <motion.div 
      initial={{ x: 300 }}
      animate={{ x: 0 }}
      style={{ 
        position: "fixed", top: 20, left: 20, right: 20, zIndex: 4000,
        background: "#fff", borderRadius: 20, padding: 20, border: "2px solid #1E88E5",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 900, color: "#0A1628" }}>{request.userName} needs a Buddy</div>
          <div style={{ fontSize: 12, color: "#666" }}>Walking through <strong>{request.district}</strong>. Watch his back?</div>
        </div>
      </div>
      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={onReject} style={{ flex: 1, padding: 12, borderRadius: 12, border: "none", background: "#F5F4F0", color: "#666", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Decline</button>
        <button onClick={onAccept} style={{ flex: 2, padding: 12, borderRadius: 12, border: "none", background: "#1E88E5", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Accept & Watch</button>
      </div>
    </motion.div>
  );
}
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where("buddyEmail", "==", user.email.toLowerCase()),
  ...
  useEffect(() => {
    if (activeBuddyRequest && activeBuddyRequest.lastPulseAt) {
      const lastPulse = activeBuddyRequest.lastPulseAt.toMillis ? activeBuddyRequest.lastPulseAt.toMillis() : activeBuddyRequest.lastPulseAt;
  ...
  useEffect(() => {
    if (incidents.length > 0 && user) {
      ...
      const isNew = latest.timestamp?.toMillis ? latest.timestamp.toMillis() > Date.now() - 30000 : false;
  ...
  useEffect(() => {
    ...
    const unsubInc = onSnapshot(qIncidents, (snap) => {
      ...
      const userDistrict = getUserDistrict(userPosRef.current);
      const scored = data.map((incident: any) => {
        let score = 0;
        const now = Date.now();
        const timestamp = incident.timestamp?.toMillis ? incident.timestamp.toMillis() : 0;
        ...
        if (incident.type === 'welfare' && incident.bloodType) score += 600;
        ...
        // Proximity to User
        if (userPosRef.current && incident.location) {
          const dist = getDistance(userPosRef.current.lat, userPosRef.current.lng, incident.location.lat, incident.location.lng);
        ...
        userProfile?.sosContacts?.forEach((c: any) => {
          if (c.district && incident.district === c.district) {
            score += 1200; // High priority for reports in contact locations
          }
        });
        ...
        contactSosSignals.forEach((sig: any) => {
          if (incident.location && sig.lat && sig.lng) {
            ...
            if (dist <= 3000) score += 2500; // Extreme priority if near a contact in SOS
          }
        });
  ...
  return (
    <div style={{ fontFamily: "'DM Sans','Helvetica Neue',sans-serif", background: "#F5F4F0", minHeight: "100vh", maxWidth: 430, margin: "0 auto", position: "relative", overflow: "hidden" }}>
      ...
      <AnimatePresence>
        ...
        {incomingBuddyRequests.filter(r => r.status === "pending").map(req => (
          <BuddyRequestAlert 
            key={req.id} 
            request={req} 
            onAccept={async () => {
              try {
                await updateDoc(doc(db, "buddy_requests", req.id), { 
                  status: "active", 
                  buddyUid: user.uid, 
                  buddyName: user.displayName || "Buddy" 
                });
              } catch (e) {
        ...
        {incomingBuddyRequests.filter(r => r.status === "active").map(req => (
          <BuddyWatchOverlay 
            key={req.id}
            request={req} 
            user={user} 
            ...
            onStop={async () => {
              try {
                await updateDoc(doc(db, "buddy_requests", req.id), { status: "completed" });
              } catch (e) {
                handleFirestoreError(e, OperationType.UPDATE, `buddy_requests/${req.id}`);
              }
            }}
      ...
      <div style={{ position: "relative", height: "calc(100vh - 140px)" }}>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {tab === "home" && <HomeTab setTab={setTab} setDist={(n: string) => { setSelDistName(n); setTab("map"); }} incidents={incidents} news={news} liveAlerts={liveAlerts} aiInsights={aiInsights} geofences={geofences} currentUid={user?.uid} isAdmin={userRole === "admin"} onEdit={handleEdit} onDelete={handleDelete} user={user} userPos={userPos} userProfile={userProfile} activeSosId={activeSosId} onActiveSosId={setActiveSosId} polls={polls} contactSosSignals={contactSosSignals} sponsors={sponsors} />}
            ...
            />)}
            {tab === "news" && <NewsTab news={news} polls={polls} sosSignals={userRole === "admin" ? allSosSignals : contactSosSignals} isAdmin={userRole === "admin"} currentUid={user?.uid} onEnter={() => setLastViewedNews(Date.now())} incidents={incidents} sponsors={sponsors} />}
            {tab === "profile" && (user ? (
              <ProfileTab 
                user={user} 
                userRole={userRole} 
